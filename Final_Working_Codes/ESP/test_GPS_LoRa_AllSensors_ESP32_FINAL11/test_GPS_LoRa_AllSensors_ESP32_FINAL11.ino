...
const char* thingsboardToken = "YOUR_TOKEN"; // <-- PASTE YOUR TOKEN HERE
...
volatile char loraBuffer[256]; // Buffer to hold received LoRa data
...
// Geolocation variables (made global to be accessible everywhere)
double latitude = 0.0;
double longitude = 0.0;
double accuracy = 0.0;

// Vehicle state variables (new)
String vehicleStatus = "SAFE"; // Initialized to "SAFE"
...
void loop() 
{
  ...
  if (loraReceivedFlag)
  {
    String receivedLoRaMessage = String((char*)loraBuffer);
    Serial.println("-------------------------------------------");
  ...
  if (millis() - lastPublishTime >= PUBLISH_INTERVAL_MS) {
    publishTelemetry(); // No more conditional publishing
...
void publishTelemetry() {
  if (mqttClient.connected()) {
    // Increased buffer size to accommodate new fields
    ...
    // Add geolocation data to the JSON payload
    jsonDoc["latitude"] = latitude;
    jsonDoc["longitude"] = longitude;
    jsonDoc["accuracy"] = accuracy;
    
    // Add vehicle status to the JSON payload
    jsonDoc["vehicleStatus"] = vehicleStatus;

    char payload[512]; // Increased payload buffer size
    serializeJson(jsonDoc, payload);
    
    // Print geolocation and vehicle status to the serial monitor
    Serial.println("Vehicle Status: " + vehicleStatus);
...
void onReceive(int packetSize)
{
  if (packetSize > 0 && packetSize < sizeof(loraBuffer))
  {
    int i = 0;
    ...
    loraBuffer[i] = '\0'; // Null-terminate the string
...
void sendLoRaAlert(String alertMessage) {
  if (millis() - lastAlertTime >= ALERT_COOLDOWN_INTERVAL) {
    // Corrected the serial print message
...
// Updated to set the global vehicleStatus variable
void checkCriticalStatus() {
  if (temp > 30.0) {
    ...
    vehicleStatus = "CRITICAL";
  } else if (soundValue > 550) {
    sendLoRaAlert("ALERT: CRASH DETECTED! Loud noise detected!");
    vehicleStatus = "CRITICAL";
  } else {
    // If none of the critical conditions are met, the status is SAFE
    vehicleStatus = "SAFE";
  }
}
...
// Updated to save the geolocation data to global variables
void updateGeolocation() {
